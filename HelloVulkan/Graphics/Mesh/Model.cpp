...
// TODO: Replace with standard purple black checkerboard
...
void Model::LoadSlotBased(const std::string& path)
{
	bindlessTexture_ = false;

	// In case a texture type cannot be found, replace it with a black 1x1 texture
	uint32_t black = 0xff000000;
	AddTexture(BLACK_TEXTURE, (void*)&black, 1, 1);

	// Load model here
	MeshSceneData dummySceneData = {};
	LoadModel(
		path,
		dummySceneData);

	// Slot-based rendering
	CreateModelUBOBuffers();
}
...
void Model::Destroy()
{
	...
	for (Texture2D& tex : textureList_)
	{
		tex.Destroy();
	}
}

void Model::CreateModelUBOBuffers()
{
	constexpr uint32_t frameCount = MAX_FRAMES_IN_FLIGHT;
	modelBuffers_.resize(frameCount, {gd});
...
void Model::SetModelUBO(ModelUBO ubo)
{
	for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
	{
		modelBuffers_[i].UploadBufferData(&ubo, sizeof(ModelUBO));
	}
}

void Model::AddTexture(const std::string& textureFilename)
{
	textureList_.emplace_back(gd);
	const std::string fullFilePath = this->directory_ + '/' + textureFilename;
	textureList_.back().CreateImageResources(fullFilePath);
	textureMap_[textureFilename] = static_cast<uint32_t>(textureList_.size() - 1);
}

void Model::AddTexture(const std::string& textureName, void* data, int width, int height)
{
	textureList_.emplace_back(gd);
...
Texture2D* Model::GetTexture(uint32_t textureIndex)
{
	...
	MeshSceneData& sceneData)
{
	...
	if (processAnimation_) { sceneData.boneMatrixCount_ += MAX_SKINNING_MATRICES; }
}
	...
	const glm::mat4& transform)
{
	...
	std::vector<DefaultVertexData> vertices = GetMeshVertices(mesh, processAnimation_ ? glm::mat4(1.0f) : transform);
	std::vector<uint32_t> indices = GetMeshIndices(mesh);
	std::unordered_map<TextureType, uint32_t> textures = GetMeshTextures( mesh);
	...
	uint32_t prevVertexOffset)
{
	...
	for (uint32_t i = 0; i < vertexCount; ++i)
	{
		...
		for (uint32_t j = 0; j < MAX_SKINNING_BONES; ++j)
		{
	...
	const aiMesh* mesh)
{
	...
	for (uint32_t b = 0; b < mesh->mNumBones; ++b)
	{
		...
		for (uint32_t w = 0; w < numWeights; ++w)
		{
			...
			for (uint32_t iter = 0; iter < MAX_SKINNING_BONES; ++iter)
			{
...
std::vector<DefaultVertexData> Model::GetMeshVertices(const aiMesh* mesh, const glm::mat4& transform)
{
	std::vector<DefaultVertexData> vertices;
	for (unsigned int i = 0; i < mesh->mNumVertices; ++i)
	{
		// Positions
		DefaultVertexData vertex =
		{
...
std::unordered_map<TextureType, uint32_t> Model::GetMeshTextures(const aiMesh* mesh)
{
	// PBR textures
